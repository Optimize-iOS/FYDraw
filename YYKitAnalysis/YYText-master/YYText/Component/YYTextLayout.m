...
//
//  在此类中实现根据给定的 NSString 字符串 | 具体实现参数 YYTextContainer 参数来进行
//  内容的绘制工作
//
...
+ (YYTextLayout *)layoutWithContainer:(YYTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    ...
    //在 YYTextLine 中的参数 | 在绘制时采用 rect 绘制范围
    ...
    // 设置版本号来做 API 接口
    // 根据系统的版本号来设置当前 接口兼容实现类
    // 采用 dispatch_one 形式来保证当前代码执行一次
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
    ...
    layout.range = range;
    isVerticalForm = container.verticalForm; //垂直形式
    
    // set cgPath and cgPathBox
    // 设置 （1）path 绘制每一个 Line|Run 绘制路径  （2）pathBox 绘制 Line|Run 的绘制区域
    if (container.path == nil && container.exclusionPaths.count == 0) {
        ...
        ///iOS 10 采用 CTFramesetterCreateFrame 创建 Frame 使用更大点的约束
        ...
        //获取在 Core Text 在创建 CTFrameRef 参数 Path
        cgPath = path;
    }
    if (!cgPath) goto fail;
    
    // frame setter config
    //设置 Core Text 中 Attributes 参数字段
    ...
    //获取生成当前 CTFrame 的行数 | 获取当前所有行数的开始 Start 的位置点
    //
    if (lineCount > 0) {
    ...
    for (NSUInteger i = 0; i < lineCount; i++) {
        ...
        CGPoint position; //CoreText 坐标系转换到 UIKit 的坐标系
        //cgPathBox 是什么？ cgPathBox是绘制 line|run 的 rect
    ...
    //释放在在绘制中所用的 CFTypeRef  |申请结构体|数据指针|
fail:
...
static void YYTextDrawRun(YYTextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
    ...
    if (!isVertical && !glyphTransformValue) { // draw run 绘制块
        ...
        CGContextSaveGState(context); {
            ...
            if (isVertical) {
                ...
                for (YYTextRunGlyphRange *oneRange in runRanges) {
                    ...
                    YYTextRunGlyphDrawMode mode = oneRange.drawMode;//绘制方向
                    ...
                    for (NSUInteger g = range.location; g < rangeMax; g++) {
                        ...
                        CGContextSaveGState(context); {
                            ...
                            if (YYTextCTFontContainsColorBitmapGlyphs(runFont)) {
                                ///包含颜色字体符号
                                ///绘制修改当前 CGContext 的字体、文本大小和文本矩阵
...
static void YYTextDrawText(YYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    /* 将当前图形状态的副本r推入到图形状态堆栈
     *
     * 每一个图形上下文维护一个图形状态的堆栈。并非当前绘图环境所有方面都是图形状态的元素。
     * 例如：保存的图形这状态参数
