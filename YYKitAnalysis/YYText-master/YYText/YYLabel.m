...
- (void)_setLayoutNeedRedraw {
    //把当前 Layer 标记为需要刷新
    //need display
    //设置当前 View 进行界面刷新 --> YYTextAsyncLayer(display) --> YYTextAsyncLayerDisplayTask(display) --> YYLabel中Task(YYTextAsyncLayerDisplayTaskDelegate) --> YYTextLayout(drawInContext 绘制)
...
- (void)_startLongPressTimer {
    ...
    // 创建定时器 NSTimer
    // 添加定时器在 NSRunLoopCommonModes
    //自定义 Proxy 委托对象 来解决在循环引用中问题
...
- (void)_trackDidLongPress {
    [self _endLongPressTimer]; //取消 定时器
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    //把不透明的属性设置为 NO
    //就不怕在 Layer 层次在合成过程中 额外消耗 GPU 来做计算吗???
    self.opaque = NO;
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    //在 YYLabel | YYTextView 中的内容改变时 就是调用该方式
    //TODO
...
- (YYTextAsyncLayerDisplayTask *)newAsyncDisplayTask {
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        if (layoutNeedUpdate) {
            //跟新新的 Layout 按照给定的 NSAttributedString 来计算布局
