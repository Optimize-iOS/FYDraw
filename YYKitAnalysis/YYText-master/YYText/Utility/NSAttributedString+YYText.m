...
///添加图片实现富文本布局 | String 和 Image 来进行混排
+ (NSMutableAttributedString *)yy_attachmentStringWithContent:(id)content
                                                  contentMode:(UIViewContentMode)contentMode
    ...
    //在 Text 上添加临时的附件 | 此附件可以是 UIImage|UIView|CALayer
    YYTextAttachment *attach = [YYTextAttachment new];
    attach.content = content;
    attach.contentMode = contentMode;
    [atr yy_setTextAttachment:attach range:NSMakeRange(0, atr.length)];
    ...
    //上面👆初始化 ascent | 
    //设置 当前图片 CTRunDelegateRef 图片占用的委托回调
    CTRunDelegateRef delegateRef = delegate.CTRunDelegate;
    [atr yy_setRunDelegate:delegateRef range:NSMakeRange(0, atr.length)];
    if (delegate) CFRelease(delegateRef);
...
- (void)yy_setAttribute:(NSString *)name value:(id)value range:(NSRange)range {
    //设置当前 addAttribute:value:range:
    //[self addAttribute: value: range:]
...
//设置 kCTForegroundColor | NSForegroundColor
...
//对 Text 中包含的
/*
 * ASCII 一种字符编码标准，是个英文字符集；
 * Unicode 在语言处理过程中存在中文、韩文和日文等等所以出现了：UTF-8/UTF-16/UTF-32；
 * Emoji 是 Unicode 中一部分，特定的 Emoji 表情符号对应特定的 Unicode 字符
 * 参考表：
