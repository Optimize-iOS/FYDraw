...
/**
 参考资料：https://aotu.io/notes/2016/11/07/apng/index.html
 
 PNG：4 部分组成 --> | PNG Signature | IHDR | IDAT | IEND |
 （1）PNG Signature：签名
 （2）IHDR：图像头部块
 （3）IDAT：图形数据块
 （4）IEND：图像结束块
 
 APNG：是基于 PNG 拓展来实现的，在 PNG 四个块的基础上又添加三个块区分。
     | PNG Signature | IHDR | acTL ||| fcTL | IDAT ||| fcTL | fdAT ||| fcTL | fdAT ||| IEND |
 
 （1）acTL：动画控制块 | 必须在第一个 IDAT 之前，用于告诉解析器该资源是动画 PNG，其中包含帧的总数和循环次数的信息
 （2）fcTL：帧控制块 | 在每一帧前都是必须的，出现在 IDAT 或者 fdAT 之前，包含帧序号、宽高、帧位置和延迟等信息
 （3）fdAT:帧数据块 | 和 IDAT 有相同的数据结构，除去 fcTL 中的顺序号
 
 
 在第一帧显示的过程中第一帧显示的图片信息，就是我们常见的 PNG 图片内容，当该容器不支持 APNG 图片格式时就显示第一帧的图片内容。
 
 APNG 帧之间的优化
 APNG 通过算法计算帧与帧之间的区别，然后通过算法仅仅储存每一帧之间的不同之处，不用储存当前帧的全部内容。实现保存较小内容，来节省储存空间。
 **/
...
///对图片进行解码复制返回渲染在 UI 界面上 imageRef 图片
//根据对 image 的需求解码用来显示展示 image 以及通过 ImageRef 生成的 Provider 来提供 Image
//（1）解码展示显示 Image 通过 image 基本参数初始化生成 context，然后采用 context 来 drawImage
//（2）根据展示 imageRef 生成对应 provider，然后通过 provider 来组装生成 CGImageCreate 生成对应的 imageRef
///
CGImageRef YYCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        //获取图片基本信息：颜色空间、单个颜色分配位、单个像素占用内存、位图单行占用内存和文图的信息
...
///图片资源解码过程
//（1）判断当前图片资源类型
//（2）根据当前图片类型不同采用图片解码方式 //TODO 需要在看一遍
// (3)
///
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    ///根据文件的后缀判断 image 的类型
    YYImageType type = YYImageDetectType((__bridge CFDataRef)data);//检测 imageType
    ///
...
///获取解码数据 data 的角标 index image 内容 | 在此步骤中通过解码 Image 的基本参数结合相对应的操作条件采用 CGContext 来对图片资源重新绘制
//（1）获取解码数据中图片基本参数 decoderFrame；
//（2）图片不需要融合就通过获取不融合 imageRef 根据是否需要解码生成对应的 imageRef 然后装配 Image 赋值给 decoderFrame 参数；
//（3）图片需要融合需要判断三种情况：(a)当前帧是上次融合下一帧 (b)当前帧 (c)
//（4）
///
...
- (void)_updateSource { //按照具体类型更新图片资源
    switch (_type) {
        case YYImageTypeWebP: {
            ...
            //
            //
            [self _updateSourceWebP];//更新 WebP 的数据
        } break;
        ...
        case YYImageTypePNG: {
            ...
            //
            //
            [self _updateSourceAPNG];//更新 APNG 的数据
        } break;
            
        default: {
            ///解码非 WebP/APng 的图片格式及其步骤
            //（1）通过资源文件 data 获取对应的 image source；
            //（2）对 image source 资源判断其 frame count 然后对每一帧图片进行解析获取图片基本信息；
            //（3）解析过程针对 Gif 格式图片设置其持续 duration 时间
            [self _updateSourceImageIO];//更新 其余格式 的数据
        } break;
    }
}
...
///对于非 WebP 和 APNG 数据解析
//（1）创建 ImageSource；
//（2）根据资源类型来确定当前 loopCount 次数；
//（3）根据 frameCount 获取解析每一帧 Image 参数。
//
///
...
///根据 index 角标获取没有混合 image
//（1）当前图片的资源 image source 存在时就采用 source 中 index 获取相对应的图片资源，然后对改图片资源进行 decode display 解码展示
//（2）
//（3）
///
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    ///三种情况：（1）image source (2)png source (3)web source
    if (_source) {
        //通过 image sorce 生成 imageRef
        ...
        if (imageRef && extendToCanvas) {
            size_t width = CGImageGetWidth(imageRef);
            size_t height = CGImageGetHeight(imageRef);
            ///图片资源 width | height 相等时对解码后的图片资源进行 copy 一份
            //图片资源 width | height 不相等时就采用 CGContextDrawImage 重新绘制一遍，对图片宽高调整
            if (width == _width && height == _height) {
                ...
                //根据创建的 bitmap 的上下文获取生成的 imageRef
    ...
    if (_apngSource) {
        ...
        ///根据数据生成 provider,通过 provider 生成对应的 image source, 最后通过 image source 生成对应的 image ref
        ///然后在上下文中绘制当前 Image
...
- (CGImageRef)_newBlendedImageWithFrame:(_YYImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    ...
    ///
    //三种场景的对应不同的实现形式：
    //（1）在当前帧生成之前显示 Previous 帧，在绘制之前获取前，绘制当前帧，清除当前绘制空间，绘制 previous image
    //（2）获取 current image，然后在上下文中进行绘制，清除当前上下文绘制的空间
    //（3）获取当前帧复制给 frame 的拥有者，后续不做任何操作
    ///
    if (frame.dispose == YYImageDisposePrevious) {
        if (frame.blend == YYImageBlendOver) {
            CGImageRef previousImage = CGBitmapContextCreateImage(_blendCanvas);
            CGImageRef unblendImage = [self _newUnblendedImageAtIndex:frame.index extendToCanvas:NO decoded:NULL];
            ///在图片混合中覆盖原来的内存 | 在当前上下文直接绘制 image 图片内容 |
            if (unblendImage) {
                CGContextDrawImage(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height), unblendImage);
                CFRelease(unblendImage);
            }
            ...
            CGImageRef previousImage = CGBitmapContextCreateImage(_blendCanvas);
            CGImageRef unblendImage = [self _newUnblendedImageAtIndex:frame.index extendToCanvas:NO decoded:NULL];
            ///清楚在原来内容在进行 image 内容绘制 | 清除后直接绘制 image 图片内容 |
            if (unblendImage) {
                //在不加融合请款下，清除上下文绘制空间
                //添加融合就先清除绘制区域，然后在绘制显示的内容
                ///
                CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
                CGContextDrawImage(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height), unblendImage);
                CFRelease(unblendImage);
        ...
        if (frame.blend == YYImageBlendOver) {
            ///获取对应的图片资源 | 直接进行绘制 | 绘制结束后清除当前的上线文
            CGImageRef unblendImage = [self _newUnblendedImageAtIndex:frame.index extendToCanvas:NO decoded:NULL];
            if (unblendImage) {
                CGContextDrawImage(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height), unblendImage);
                CFRelease(unblendImage);
        ...
        if (frame.blend == YYImageBlendOver) {
            ...
            imageRef = CGBitmapContextCreateImage(_blendCanvas);
            ///获取对应的图片直接进行绘制 | 不做任何对上下文清除或者是复制操作 |
        } else {
            CGImageRef unblendImage = [self _newUnblendedImageAtIndex:frame.index extendToCanvas:NO decoded:NULL];
            if (unblendImage) {
                CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
